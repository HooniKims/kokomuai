...
import type { UsageSurface } from "../src/domain/usage/usageAccounting";
...
export function createChatResponsePlan(requestBody: ChatRequest): ChatResponsePlan {
  ...
  const guardDecision = classifyStudentMessage(requestBody.message, requestBody.chatbot);
  ...
  if (guardDecision.kind === "privacy_risk") {
    return {
      ...
      payload: {
        error: "privacy_risk",
        message: "개인정보로 보이는 내용이 있어요. 이름, 학번, 전화번호, 주소 같은 정보는 빼고 다시 적어 주세요.",
  ...
  if (guardDecision.blockAiCall) {
    return {
      kind: "guardrail",
      assistantMessage: createGuardrailReply(guardDecision, requestBody.chatbot),
      guardDecision
    };
  }
  ...
  return {
    ...
    guardDecision,
    messages: [
      { role: "system", content: buildStudentSystemPrompt(requestBody.chatbot) },
      ...safeHistory,
