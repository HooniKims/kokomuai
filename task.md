...
- 학생 수준에 비해 추상적인 교육과정 용어가 질문에 직접 노출될 수 있었다.
- 힌트가 문장 안에 섞여 학생이 무엇을 붙잡아야 하는지 알아보기 어려웠다.
- 학습 전, 수업 중, 수업 후에 맞는 질문 난이도를 교사가 조절할 수 없었다.

해결:

- 챗봇 생성 폼의 힌트 강도 오른쪽에 `질문 수준` 선택을 추가했다.
- `쉽게`, `보통`, `어렵게` 수준을 시스템 프롬프트에 반영해 각각 학습 전, 수업 중, 수업 후 질문으로 작동하게 했다.
- 쉬운 수준에서는 개념을 모르는 학생도 답할 수 있도록 쉬운 말, 관찰 질문, 한 단계 질문을 우선하도록 했다.
- AI 응답의 핵심 단어, 비교 기준, 관찰 포인트는 Markdown 굵게 표시하도록 정책을 추가했다.
- `음운 체계와 연결해서 말해 보세요`처럼 추상적인 말은 학생에게 직접 묻지 않고, 구체적인 관찰 질문으로 바꾸도록 금지 규칙을 추가했다.

### 2022 교육과정 기반 수업 주제 추천

변경:

- 챗봇 생성 폼의 `수업 주제` 아래에 2022 교육과정 추천 성취기준을 바탕으로 한 주제 예시 칩을 추가했다.
- 교사가 추천 주제를 누르면 입력칸에 바로 적용된다.
- 추천 주제는 학교급, 학년군, 과목, 영역, 성취기준 핵심 표현을 조합해 만든다.

검증:

- 질문 수준별 시스템 프롬프트 정책 테스트 추가
- 굵은 힌트와 추상 용어 회피 정책 테스트 추가
- 2022 교육과정 기반 수업 주제 추천 테스트 추가
- 교사 챗봇 생성 화면의 질문 수준 선택과 주제 추천 적용 테스트 추가
- `npm run build` 통과
- `npm test -- --run tests/domain/chatPolicy.test.ts tests/presentation/chatbotFormSuggestions.test.ts tests/presentation/teacherChatbotCreationFocus.test.ts` 통과
- `npm test -- --run tests/presentation tests/domain tests/application` 통과
...
- 모바일 관리자 화면에서 AI 모델 선택 영역 설명이 한 글자씩 세로로 길게 깨지는 문제를 수정한다.

원인:

- 관리자 AI 설정 카드가 모바일/태블릿 폭에서도 2열 그리드를 유지했다.
- 오른쪽 모델 선택 칸의 최소 폭 때문에 왼쪽 설명 영역이 지나치게 좁아져 한국어 문장이 한 글자 폭으로 눌렸다.

수정:

- 920px 이하 화면에서 `.admin-ai-settings`를 1열로 전환했다.
- 모델 선택 `label`과 `select`에 `min-width: 0`을 적용해 작은 화면에서 내부 컨트롤이 레이아웃을 밀어내지 않게 했다.
- 관리자 AI 설정 스타일 회귀 테스트를 추가했다.
...
- 전체 테스트
  - `npm test`
  - 결과: 통과
  - 78개 테스트 파일, 347개 테스트 통과
- 빌드
  - `npm run build`
  - 결과: 통과
- 브라우저 반응형 점검
  - Playwright로 320px, 390px, 768px 폭에서 관리자 모델 설정 카드 CSS를 검증했다.
  - 모델 설정 카드가 1열로 표시되고 가로 오버플로가 없음을 확인했다.
...
- 페르소나 입력칸의 플레이스홀더가 특정 과목의 국어 선생님으로 보이지 않게 한다.
- 기본 안내 문구를 `친절하지만 답을 바로 말하지 않고 예문과 질문으로 이끄는 선생님`으로 표시한다.

수정:

- `src/presentation/teacherChatbotSample.ts`의 페르소나 샘플 문구에서 `국어`를 제거했다.
- 플레이스홀더 회귀 테스트를 정상 한글 문구 기준으로 정리했다.

검증:

- 관련 테스트
  - `npm test -- tests/presentation/teacherChatbotSample.test.ts`
  - 결과: 통과
- 전체 테스트
  - `npm test`
  - 결과: 통과
  - 77개 테스트 파일, 341개 테스트 통과
- 빌드
  - `npm run build`
  - 결과: 통과
...
- 관리자 대시보드에서 교사별 접근, 전체 챗봇 운영, 사용량 순위를 토글로 확인한다.
- 사람별 분류와 검색으로 교사/챗봇/사용량을 찾을 수 있게 한다.
- 회원가입 중 브라우저 뒤로가기를 누르면 외부 페이지로 빠지지 않고 로그인 화면으로 돌아오게 한다.
- 챗봇 이름과 아래 입력 내용을 수정해도 이미 선택한 성취기준이 사라지지 않게 한다.
- 배포 환경에서 `/api/chatbots` 생성 요청이 500으로 실패하는 문제를 수정한다.

원인:

- 관리자 화면의 교사별 챗봇, 전체 챗봇, 사용량 확인이 별도 섹션으로 흩어져 있어 검색/분류 기준이 없었다.
- 로그인/가입 화면 전환이 React 상태에만 있었고 브라우저 히스토리에는 반영되지 않았다.
- 성취기준 선택은 ID만 저장했고, 추천 목록이 새 입력값 기준으로 갱신되면 선택한 추천 원본이 화면 목록에서 빠졌다.
- 챗봇 주제 검증이 `1차 함수`처럼 짧지만 구체적인 교과 개념까지 막고 있었고, 검증 오류가 400이 아니라 일반 500으로 감싸졌다.

수정:

- 관리자 운영 패널에 `교사별 접근`, `전체 챗봇 운영`, `사용량 순위` 토글을 추가했다.
- 관리자 운영 패널에 검색 입력과 사람별 분류 필터를 추가했다.
- 교사별 접근 화면에서 사람별 챗봇 수, 대화 수, 토큰, 예상 비용, 보유 챗봇을 함께 볼 수 있게 했다.
- 사용량 순위는 대화 수, 토큰 수, 예상 비용 기준으로 정렬한다.
- 회원가입 전환 시 브라우저 히스토리에 `authMode`를 기록하고, `popstate`에서 로그인 화면으로 복귀하도록 했다.
- 선택한 성취기준 추천 항목을 캐시에 보관하고, 추천 결과가 바뀌어도 선택 항목을 목록 앞에 고정했다.
- 챗봇 주제 검증에서 임의의 6자 제한을 제거하고, `1차 함수` 같은 짧은 개념을 허용했다.
- 챗봇 생성 검증 실패는 `invalid_chatbot_draft` 400 응답으로 반환하도록 했다.

검증:

- 관련 테스트
  - `npm test -- tests/domain/chatbotManagement.test.ts tests/infrastructure/localApiAuth.test.ts tests/presentation/curriculumSelection.test.ts tests/presentation/adminOperationsPanel.test.ts tests/presentation/authHistory.test.ts`
  - 결과: 통과
  - 5개 테스트 파일, 41개 테스트 통과
- 전체 테스트
  - `npm test`
  - 결과: 통과
  - 77개 테스트 파일, 341개 테스트 통과
...
### 운영 전환 53차: E2B reasoning 패턴 확장과 초기 스트리밍 지연 완화

완료 시간: 2026-06-14 21:36:00 +09:00

요청:

- 시스템 프롬프트를 넣어도 E2B가 `현재 목표`, `다음 행동 계획`, `힌트 강도` 같은 추론 과정을 출력하는 문제를 막는다.
- 응답 초반이 한 덩어리로 나타난 뒤부터 스트리밍되는 현상을 완화한다.

수정:

- 숨김 reasoning 패턴에 `현재 목표`, `다음 행동 계획`, `힌트 강도`, `학생은 ... 어렵게 느꼈습니다` 형태를 추가했다.
- 숨김 reasoning 뒤에 `아이고,`, `네, 그럼`, `좋아요.` 같은 실제 학생용 답변 시작이 붙어 있으면 그 지점부터 다시 출력하도록 했다.
- 정상 답변의 초기 버퍼 검사 길이를 256자에서 16자로 줄여 초반 스트리밍 지연을 줄였다.
...
### 운영 전환 56차: PDF 한글 깨짐 수정과 파일 생성 전체 화면 안내

완료 시간: 2026-06-14 22:07:00 +09:00

요청:

- 학생 채팅 PDF를 내려받으면 한글 내용이 깨지는 문제를 수정한다.
- PDF 또는 TXT 파일을 만드는 동안 사용자가 기다릴 수 있게 안내 메시지를 보여준다.
- 파일 생성 안내는 전체 화면으로 표시한다.

원인:

- 기존 PDF 생성 방식은 `jsPDF` 기본 폰트에 텍스트를 직접 쓰는 구조였다.
- `jsPDF` 기본 폰트는 한글 글리프를 포함하지 않아 PDF 안의 한국어가 깨질 수 있었다.

수정:

- 학생 채팅 내보내기 로직을 `src/presentation/chatExport.ts`로 분리했다.
- TXT 내보내기는 기존처럼 UTF-8 텍스트 파일로 저장하되, 한글 문구가 깨지지 않도록 테스트를 추가했다.
- PDF 내보내기는 브라우저가 한글을 정상 렌더링한 HTML을 `html2canvas`로 캡처한 뒤, 그 이미지를 `jsPDF` 페이지에 나누어 넣는 방식으로 변경했다.
- PDF/TXT 생성 버튼을 누르면 `PDF 파일을 만드는 중입니다.` 또는 `TXT 파일을 만드는 중입니다.` 안내가 전체 화면 오버레이로 표시되도록 했다.
- 생성 중에는 전체 화면 오버레이가 떠 있어 사용자가 다른 조작을 하지 않고 기다릴 수 있게 했다.
