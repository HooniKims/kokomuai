...
import { shouldShowRoleNavigation, shouldUseFirebaseTeacherAuth } from "../../src/presentation/App";
...
describe("student share navigation", () => {
  ...
  it("uses Firebase teacher auth only on operator pages when Firebase is configured", () => {
    ...
    expect(shouldUseFirebaseTeacherAuth("/s/public-token", true, true)).toBe(false);
